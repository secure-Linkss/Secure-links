...
def create_link():
    ...
    try:
        ...
        if domain_type == "vercel":
            ...
            shortio_api_key = os.environ.get("SHORTIO_API_KEY") # You need to set this environment variable
            if not shortio_api_key:
                return jsonify({"success": False, "error": "Short.io API key not configured"}), 500
            
            shortio_domain = os.environ.get("SHORTIO_DOMAIN") # You need to set this environment variable
...
def get_links_stats():
    ...
    try:
        ...
        avg_ctr = 0
        if total_links > 0:
            avg_ctr = (total_clicks / total_links) * 10 if total_clicks > 0 else 0  # Simplified CTR calculation
        
        return jsonify({
            "totalLinks": total_links,
            "totalClicks": total_clicks,
            "activeLinks": active_links,
            "avgCTR": min(avg_ctr, 100)  # Cap at 100%
        })
